  ...
  <section class="experts-wrap">
    <div class="website-container">
      <div class="search-wrap">
        <input type="text" v-model="keyword" @keyup.13="go" autofocus="autofocus">
        <button type="button" @click="go">搜索</button>
      </div>
      <ul class="experts">
  ...
  export default {
    ...
    watch: {
      async user (val, newVal) {
        try {
          const {data} = await axios.get('/webapi/v2/pageExpertInfo', {
            params: {
              offset: this.keyword || 0,
